...
class GtalkRobot:
    ...
    def controller(self, conn, message):
        ...
        if text:
            ...
            for (pattern, bounded_method) in self.commands:
                ...
                if(match_obj):
                    try:
                        ts = time.strftime("%Y-%m-%d %a %H:%M:%S", time.localtime())
                        logging.info(ts + ", user="+user.getStripped() + ", command=" + text)
                        ...
                        logging.error(ts + traceback.format_exc())
                        self.replyMessage(user, "Hmmm...something went wrong there!")
    ...
    def __init__(self, server_host="talk.google.com", server_port=5223, debug=[]):
        ...
        logging.basicConfig(filename='gtalk_robot.log',level=logging.DEBUG)
